...
onMounted(() => {
  occChart = new Chart(occCanvas.value, {
    type: 'line',
    data: { labels: [], datasets: [{ label: '占用率(%)', data: [], borderColor: '#36d1ff' }] },
    options: { responsive: true, scales: { y: { min:0, max:100 } } }
  });
  utilChart = new Chart(utilCanvas.value, {
    type: 'bar',
    data: { labels: ['A','B','C','D'], datasets: [{ label: '利用率(%)', data: [0,0,0,0], backgroundColor: '#7b61ff' }] },
    options: { responsive: true, scales: { y: { min:0, max:100 } } }
  });
});

watch(() => props.store.state.charts.occupancyHistory.length, () => {
  if (!occChart) return;
...
watch(() => props.store.state.charts.utilRates, (rates) => {
  if (!utilChart) return;
  ...
  <section id="tab-overview" class="tab-panel active" aria-label="总览">
    <div class="grid">
      <div class="card span-2">
        ...
        <div class="stats">
          <div class="stat">
            ...
            <div class="stat-value">{{ props.store.state.stats.totalPeople }}</div>
          </div>
          <div class="stat">
          ...
          <div class="stat">
            ...
            <div class="stat-value">{{ props.store.state.stats.avgTemp ? props.store.state.stats.avgTemp + '°C' : '--°C' }}</div>
          </div>
          <div class="stat">
      ...
      <div class="card span-2">
        <div class="card-header">
          ...
          <div class="feeds-actions">
            <button class="btn ghost" @click="props.store.refreshFeeds">刷新画面</button>
          </div>
        </div>
        <div class="feeds">
          <figure class="feed">
      ...
      <div class="card span-2">
        ...
        <div class="courts">
          <div class="court" v-for="c in props.store.state.courts" :key="c.id" :id="`court-${c.id}`">
            <div class="court-head">
              <span class="court-name">{{ c.id }}场</span>
              <span class="badge" :data-status="c.status">{{ c.status==='busy' ? '使用中' : c.status==='idle' ? '空闲' : '维护中' }}</span>
            </div>
            <div class="court-body">
              <div>人数：<span class="value people">{{ c.people }}</span></div>
              <div>温度：<span class="value temp">{{ c.temp ? c.temp.toFixed(1) + '°C' : '--°C' }}</span></div>
              <div>湿度：<span class="value humid">{{ c.humid ? c.humid.toFixed(0) + '%' : '--%' }}</span></div>
              <div>灯光：<span class="value light">{{ c.light }}</span></div>
            </div>
            <div class="court-actions">
              <button class="btn small" @click="props.store.toggleCourtLight(c.id)">切换灯光</button>
              <button class="btn small ghost" :disabled="c.status==='maintain'">预约</button>
            </div>
          </div>
        </div>
      </div>
