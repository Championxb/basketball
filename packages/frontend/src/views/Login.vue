<template>
  <div class="container" style="height:100vh;display:flex;align-items:center;justify-content:center;">
    <div class="card" style="width:820px;max-width:95%;display:flex;overflow:hidden;border-radius:20px;">
      <!-- 视觉侧栏 -->
        ...
        <div class="visual-inner">
          <div class="brand">
            ...
            <div>
              ...
              <div class="muted" style="font-size:12px">管理与监控你的篮球场</div>
            </div>
          </div>

          <h3 style="margin-top:18px;color:var(--text)">欢迎回来</h3>
          <p class="muted" style="max-width:220px">使用账户登录以访问仪表盘、场地与设备数据。</p>
        </div>
      </div>
      ...
      <div class="login-form" style="padding:28px;flex:1;">
        ...
        <p class="muted" style="margin-top:0;margin-bottom:18px">请输入用户名和密码以继续</p>
        ...
        <form @submit.prevent="onSubmit">
          ...
          <div class="field" style="position:relative">
            ...
            <button type="button" class="btn small ghost" @click="toggleShow" style="position:absolute;right:6px;top:30px">{{ showPassword ? '隐藏' : '显示' }}</button>
          </div>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:14px;gap:12px">
            ...
            <div style="display:flex;gap:8px">
              ...
              <button type="button" class="btn ghost" @click="clear">清除</button>
            </div>
          </div>
        ...
        <div style="margin-top:20px" class="muted">还没有账号？联系<a style="color:aqua">管理员</a>创建。</div>
      </div>
    </div>
  </div>
</template>
...
async function onSubmit() {
  ...
  try {
    ...
    if (data && data.token) {
      ...
      error.value = data && data.message ? data.message : '登录失败，请检查用户名与密码'
    }
  } catch (e) {
    error.value = e && e.response && e.response.data && e.response.data.message ? e.response.data.message : '网络或服务器错误'
...
.login-visual {
  width:280px;
  background: linear-gradient(180deg, rgba(54,209,255,0.08), rgba(123,97,255,0.06));
  display:flex;
  align-items:center;
  justify-content:center;
  padding:22px;
}
.visual-inner { max-width:220px }
...
@media (max-width:700px) {
  ...
  .login-visual { width:100%; padding:16px }
}
</style>
